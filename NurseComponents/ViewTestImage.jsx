  ...
  useEffect(() => {
    ...
    const fetchPatientImages = async () => {
      try {
        ...
        const response = await axios.get(`${API_BASE_URL}/nurse/viewTestImages/${id}`,{
          headers: {
...
const handleDelete = async (testimageId) => {
  try {
    const token = await AsyncStorage.getItem('token');
      await axios.delete(`${API_BASE_URL}/nurse/deleteTestImage/${testimageId}`,{
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
      setIsLoading(true);
      navigation.navigate('ViewTestImage',{id})
  } catch (error) {
  ...
  return (
    <View style={styles.container}>
      ...
      >
        ...
        <Text style={styles.heading}>View Images: For Selected Test Results</Text>
          <TouchableOpacity style={styles.addButton} onPress={() => handleadd(id)}>
            ...
            {patientImages.map((image, index) => (
              <View key={index} style={styles.symptomImageItem}>
                ...
                <View style={styles.actionContainer}>
                                    <TouchableOpacity onPress={() => handleEdit(image.testimageId)}>
                                        <MaterialIcons name="edit" size={24} color="blue" />
                                    </TouchableOpacity>
                                    <TouchableOpacity onPress={() => handleDelete(image.testimageId)}>

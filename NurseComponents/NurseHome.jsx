...
export default function NurseHome({ navigation }) {
  ...
  const getPatientsFilledStatusCounts = () => {
    const fullyFilledCount = patients.filter((patient) => patient.colorData.symptomsFilled && patient.colorData.vitalsFilled).length;
    const partiallyFilledCount = patients.filter((patient) => patient.colorData.symptomsFilled || patient.colorData.vitalsFilled).length - fullyFilledCount;
    const notFilledCount = patients.length - (fullyFilledCount + partiallyFilledCount);
    return [fullyFilledCount, partiallyFilledCount, notFilledCount];
  };

  const getEmerPatientsFilledStatusCounts = () => {
    const fullyFilledCount = emergencyPatients.filter((patient) => patient.colorData.symptomsFilled && patient.colorData.vitalsFilled).length;
    const partiallyFilledCount = emergencyPatients.filter((patient) => patient.colorData.symptomsFilled || patient.colorData.vitalsFilled).length - fullyFilledCount;
    const notFilledCount = emergencyPatients.length - (fullyFilledCount + partiallyFilledCount);
    return [fullyFilledCount, partiallyFilledCount, notFilledCount];
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.content}>
        {isSidebarOpen && <NurseSidebar navigation={navigation} nurseId={nurseId} isSidebarOpen={isSidebarOpen} activeRoute="NurseHome"/>}
        ...
        <View style={styles.legendWrapper}>
            <View style={styles.legendContainer}>
              ...
              <View style={styles.legendItem}>
                <View style={[styles.legendColorBox, { backgroundColor: 'wheat' }]} />
...
/>
  ...
  <View style={styles.pieChartItem}>
  <Text style={styles.chartTitle}>Filled Status for Emergency Patients</Text>
  <PieChart
    data={[
      { value: getEmerPatientsFilledStatusCounts()[0], color: 'palegreen', text: `${getEmerPatientsFilledStatusCounts()[0]}`, textColor: 'teal' },
      { value: getEmerPatientsFilledStatusCounts()[1], color: 'wheat', text: `${getEmerPatientsFilledStatusCounts()[1]}`, textColor: 'teal' },
      { value: getEmerPatientsFilledStatusCounts()[2], color: 'tomato', text: `${getEmerPatientsFilledStatusCounts()[2]}`, textColor: 'teal' },
    ]}
    donut={true}
    radius={90}
    showText={true}
...
<View style={styles.pieChartItem}>
  <Text style={styles.chartTitle}>Filled Status for General Patients</Text>
  <PieChart
    data={[
      { value: getPatientsFilledStatusCounts()[0], color: 'palegreen', text: `${getPatientsFilledStatusCounts()[0]}`, textColor: 'teal' },
      { value: getPatientsFilledStatusCounts()[1], color: 'wheat', text: `${getPatientsFilledStatusCounts()[1]}`, textColor: 'indigo' },
      { value: getPatientsFilledStatusCounts()[2], color: 'tomato', text: `${getPatientsFilledStatusCounts()[2]}`, textColor: 'teal' },
    ]}
    donut={true}
    radius={90}
    showText={true}

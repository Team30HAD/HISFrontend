  ...
  const fetchOldPastImage = async () => {
      try {
        const token = await AsyncStorage.getItem('token');
          const response = await axios.get(`${API_BASE_URL}/nurse/viewPastImagesById/${historyId}/${imgId}`,{
            headers: {
    ...
    const handleImageUpload = async () => {
      try {
        ...
        if (!regex.test(pastImg)) {
          
          Alert.alert('Error', 'Image URL must be in .png or .jpg format');
          return;
        }
        const token = await AsyncStorage.getItem('token');
        const response = await axios.put(`${API_BASE_URL}/nurse/editPastImages/${imgId}`, { pastImg},{
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
...
<SemicircleBackground style={styles.rbackground}>
  <TouchableOpacity onPress={() => navigation.navigate('ViewPastImage', { patientId,historyId })} style={styles.footerItem}>

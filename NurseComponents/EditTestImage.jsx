  ...
  const fetchOldPastImage = async () => {
      try {
        const token = await AsyncStorage.getItem('token');
          const response = await axios.get(`${API_BASE_URL}/nurse/viewTestImagesById/${id}/${testimageId}`,{
            headers: {
    ...
    const handleImageUpload = async () => {
      try {
        ...
        const token = await AsyncStorage.getItem('token');
        const response = await axios.put(`${API_BASE_URL}/nurse/editTestImage/${testimageId}`, { image},{
          headers: {
            Authorization: `Bearer ${token}`,
          },
        });
...
<SemicircleBackground style={styles.rbackground}>
  <TouchableOpacity onPress={() => navigation.navigate('ViewTestImage', { patientId,id })} style={styles.footerItem}>
